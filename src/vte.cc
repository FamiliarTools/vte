                            ...
                            vte::grid::row_t amount, bool fill)
{
         ...
         * sideways within their rows, and a picture whose cells are all inside
         * the moving rectangle goes with them. Which sequences those are is a
         * grep, not a list - see doc/images.txt, "Sideways movement carries
         * the image". */
        shift_images_for_scroll(top, bottom, left, right, amount);
                      ...
                      bool invalidate_now)
{
	...
	if (G_UNLIKELY (columns == 0)) {
		 ...
		 *
		 * Read the base AFTER the choke point, not before: the erase
		 * blanks a cell it takes back from an image, and combining onto
		 * what such a cell held would carry the image's own content into
        ...
        if (m_images_enabled) {
                ...
                for (auto row = top_row; row <= bottom_row; row++) {
                        ...
                        for (auto col = 0; col < row_data->len; ) {
                                 ...
                                 * so it is addressed against the image's OWN
                                 * layout cell and not m_cell_*: an image
