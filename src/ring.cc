 ...
 *
 * What the caller of a horizontal scroll (ICH, DCH, DECIC, DECDC, SL, SR and
 * insert mode) owes the images before it memmoves the cells.
 *
 ...
 * one of whose CELLS is inside the moving rectangle, and at least one of
 * whose cells is still inside it after the move, is shifted here and keeps
 * them.
 *
 ...
 * moved, part of it standing still - and no single rectangle can describe the
 * result. Those lose the cells inside the rectangle exactly as any other
 * write to them would, which is what erase_images_in_rect() already does.
 */
bool
 ...
 * in m_image_by_top_map - so a follower is moved through reanchor_image(),
 * which is the ring's one mover that keeps those two halves in step.
 *
                               ...
                               long* damage_bottom) noexcept
{
        ...
        if (amount != 0) {
                for (auto const& [key, image] : m_image_by_top_map) {
                        ...
                        /* As in shift_images_for_scroll(): rows above
        ...
        for (auto* const image : followers) {
                 ...
                 * move together or validate_images() catches it. Only
                 * reanchor_image() moves both, and it wants the image's own
                 * node - found now rather than kept from the scan above,
                 * because the erase in between can have freed others.
                 */
                auto const range = m_image_by_top_map.equal_range(row_t(image->get_top()));
                for (auto it = range.first; it != range.second; ++it) {
                        if (it->second != image)
                                continue;

                        reanchor_image(it, row_t(long(image->get_top()) + amount));
                        break;
                }
        }
