...
# /vte/ring/image/spill-rows-survive-rewrap, and the row a restored image is
# anchored on is asserted by /vte/ring/image/spill-restore-after-rewrap. Both
# of those see the ring's data structures and nothing else. The whole path -
# draw, scroll, evict, spill, reflow, wheel back, fault in, draw again - had
# never been walked end to end on a real frame.
#
...
# test.
#
# WHAT THIS CANNOT CATCH, said plainly so nobody reads it as more than it is.
# It is coverage of the spill/restore/reflow path, not a regression guard for
# the two row-number fixes it was written alongside. The draw walks the cells
# that name an image and places each stripe from ITS OWN row and column, never
# from image->get_top(), so a stale or unrenumbered anchor still paints in the
# right place. Measured, not reasoned: with the live-map read in
# Ring::restore_image() reverted to the on-disk record.top_cells this test was
# run on both arms and stayed green, while
# /vte/ring/image/spill-restore-after-rewrap went red. The unit tests are the
# guard for the anchor; this is the proof that the path they assert about runs
# at all, in a real terminal, and puts a whole picture on the screen.
#
...
# Run against a scratch home, not the invoking developer's; see render-test.sh
# for the settings the app would otherwise pick up out of it.
...
case "$DISP" in
        0|0.*) echo "FAIL: Xvfb handed back display :$DISP, which is a real session"; exit 1 ;;
esac
...
# REQUIRED below; see confirm_spilled() and the Restored check in run_arm.
#
# Setting it has a side effect worth knowing before a kept frame is looked at:
# the freeze path reverse-videos every cell it writes to the scrollback under
# this category, so every scrolled-back frame this test captures has white text
# on black and its image at half intensity. That is why the picture is found by
# saturation and the markers are FILLED cells whose colour survives the
# inversion of a glyph.
...
# terminal's own debug output rather than inferred from the budget.
IMAGE_LIMIT=40000
...
# yet, the first row already IS zero, the old and new numbering coincide, and
# nothing about the renumbering is exercised at all. The preamble below
# overflows the scrollback before the fixture is ever drawn, so by the time the
# reflow happens the numbering has a real offset to lose.
SCROLLBACK=200
PREAMBLE=300
TRAILER=60
...
#
# These are render-test.sh's two sentinel colours, deliberately: they are
# already established there as colours no golden and no fixture contains.
TOP_HEX='#12FE34'
BOT_HEX='#DE12FE'
TOP_MARK=$'\033[38;2;18;254;52m\033[48;2;18;254;52m        \033[m'
BOT_MARK=$'\033[38;2;222;18;254m\033[48;2;222;18;254m        \033[m'

# A THIRD colour, for the paint sentinel of the readiness handshake below. It
# has to differ from both markers: the markers are measured with -fuzz 0 exact
# matches and a sentinel sharing a marker's colour would be counted as one.
PAINT_HEX='#FE7A12'
PAINT_MARK=$'\033[38;2;254;122;18m\033[48;2;254;122;18m        \033[m'
...
# DSR-5's reply, in full. Asked rather than DSR-6/CPR because it has no
# variable field, so the four bytes read back can be compared exactly rather
# than merely counted; see render-test.sh, where a count-only wait would accept
# any four bytes the child happened to be sent.
DSR_REPLY=$'\033[0n'
...
# The child, in one string.
#
# The handshake is render-test.sh's: a PROLOGUE that prints a paint sentinel,
# answers DSR-5, and then BLOCKS on a file the runner creates, so that nothing
# the verdict depends on is written until the runner has seen the terminal
# actually PAINT. render-test.sh's own note for why the reply alone is not
# enough: the terminal answers DSR-5 for bytes it has parsed, and parsed is not
# painted - measured there at up to ~300 ms apart. Here that gap would land
# somewhere worse than a golden diff, because the runner's next act is to drive
# the WHEEL and the POINTER at a window that may not exist yet.
#
# cellstore's version of this file waited only on the DSR reply, and counted
# its four bytes rather than reading them. This uses the branch's machinery on
# both counts.
child_script() {
        local work=$1
        local s=
        ...
        # Prologue: cursor off, clear, paint sentinel on the terminal's own
        # bottom row, park the cursor at home so the sentinel is not under it,
        # answer DSR-5, and wait to be released.
        s+="printf '\\033[?25l\\033[2J\\033[999;1H%s\\033[H\\033[5n' '$PAINT_MARK'; "
        s+="dd bs=1 count=4 of='$work/alive' 2>/dev/null; "
        s+="while [ ! -e '$work/go' ]; do sleep 0.02; done; "

        # Released. Wipe the sentinel off the screen before anything that is
        # measured exists, so it can never be counted as a marker or as a band.
        # The preamble then overflows the scrollback, which drops it out of the
        # ring entirely.
        s+="printf '\\033[2J\\033[H'; "
...
# render-test.sh's wait: the flag file must hold the DSR reply, ALL FOUR BYTES
# OF IT AND THOSE FOUR. A wait that only counted them would be satisfied by any
# four bytes the child was sent.
wait_for_reply() {
        local what=$1 flag=$2 why=$3
        local deadline=$((SECONDS + READY_TIMEOUT))

        while [ "$SECONDS" -lt "$deadline" ]; do
                if [ -e "$flag" ] && [ "$(wc -c <"$flag")" = 4 ]; then
                        local got
                        got=$(cat "$flag")
                        [ "$got" = "$DSR_REPLY" ] || {
                                echo "FAIL: $what: the terminal answered DSR-5 with $(cat -v "$flag"), not ESC[0n"
                                [ -s "$LOG" ] && sed 's/^/  /' "$LOG"
                                exit 1
                        }
                        return 0
...
# render-test.sh's paint wait: the sentinel colour is ON the screen, and the
# frame has then stopped changing. The colour is the signal - a frame holding a
# cell the child printed was painted from a model that already held it - and
# the repeat is belt-and-braces against a half-painted capture.
wait_for_sentinel() {
        local what=$1 hex=$2 into=$3 why=$4
        local deadline=$((SECONDS + READY_TIMEOUT))

        while [ "$SECONDS" -lt "$deadline" ]; do
                import -window root "$into" 2>"$WORK/im.log" || {
                        echo "FAIL: $what: could not capture the window"
                        exit 1
                }
                local seen
                seen=$(colour_pixels "$into" "$hex") || {
                        echo "FAIL: $what: could not look for the paint sentinel $hex"
                        exit 1
                }
...
# compiled out unless the library was built with -Ddbg=true, and a run that
# cannot see them cannot tell the two paths apart.
confirm_spilled() {
        grep -q 'New ring' "$LOG" || {
...
scroll_back_to_the_image() {
        ...
        while [ "$SECONDS" -lt "$deadline" ]; do
                ...
                # The verdict would have been taken from a picture the selection
                # had recoloured.
                #
...
run_arm() {
        ...
        "$APP" --sixel --no-load-config --no-decorations \
               ...
               -- sh -c "$(child_script "$dir")" >"$LOG" 2>&1 &
        APID=$!

        # 1. The terminal is alive and has parsed the prologue.
        wait_for_reply "$name: alive" "$dir/alive" \
                "the terminal never answered DSR-5, so it never read anything the child wrote"

        # 2. And has PAINTED it. Parsed is not painted, and everything below
        #    drives a pointer at a window that has to be on the screen.
        wait_for_sentinel "$name: painted" "$PAINT_HEX" "$dir/painted.png" \
                "the terminal never painted its window"

        # 3. Release the child. Everything measured is written after this.
        : >"$dir/go"

        # 4. The fixture, the trailer and the second image are all parsed.
        wait_for_reply "$name: drawn" "$dir/drawn" \
                "the terminal never answered DSR-5 after the images, so it never finished parsing them"
...
# gtk4's client-side border eats columns, so an 80 column request does not come
# up 80. Requiring the requested numbers would turn this test red on gtk4 for a
# reason that is not a defect. What has to be true is that the resized arm ended
# up NARROWER than the control, which is the reflow having happened at all.
